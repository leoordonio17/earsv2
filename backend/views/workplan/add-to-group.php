...
$this->title = 'Add Workplan to: ' . $group->title;
?>
<style>
    .add-workplan-container {
        padding: 30px;
...
<div class="add-workplan-container">
    <div class="page-header">
        ...
        <?= Html::a('← Back to Group', ['view-group', 'id' => $group->id], ['class' => 'btn-back']) ?>
    </div>

    <div class="form-card">
        <div class="group-info">
            <h3>📋 <?= Html::encode($group->title) ?></h3>
            <p>📅 <?= Yii::$app->formatter->asDate($group->start_date, 'php:M d, Y') ?> - <?= Yii::$app->formatter->asDate($group->end_date, 'php:M d, Y') ?></p>
            <?php if ($group->description): ?>
                <p><?= Html::encode($group->description) ?></p>
            <?php endif; ?>
        </div>
