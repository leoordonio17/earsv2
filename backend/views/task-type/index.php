...
$this->title = 'Task Type Management';
?>
<style>
    .task-type-container {
        padding: 30px;
    ...
    .task-type-table thead th:first-child {
        border-radius: 10px 0 0 0;
    }

    .task-type-table thead th:last-child {
        border-radius: 0 10px 0 0;
    }

    .task-type-table tbody tr {
        transition: all 0.3s ease;
    }

    .task-type-table tbody tr:hover {
        background: #f8f5f1;
        transform: scale(1.01);
    }

    .task-type-table tbody td {
        padding: 16px;
    ...
    .task-type-table tbody tr:last-child td {
        border-bottom: none;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const tbody = document.querySelector('.task-type-table tbody');
    if (tbody) {
    ...
    // Task Type filter functionality
...
function applyFilters() {
    ...
    const container = document.querySelector('.task-type-table-container');
    
    // If both filters are empty/default, show all rows
    if (searchTerm === '' && taskTypeFilter === 'all') {
    ...
    allRows.forEach(row => {
        const taskTypeName = row.cells[1]?.textContent.toLowerCase() || '';
        const description = row.cells[2]?.textContent.toLowerCase() || '';
        const taskTypeCell = row.cells[5];
        const taskTypeValue = taskTypeCell?.getAttribute('data-task-type') || '';

        // Check search term (if empty, matches all)
        const matchesSearch = searchTerm === '' || taskTypeName.includes(searchTerm) || description.includes(searchTerm);

        // Check task type filter
        let matchesTaskType = true;
        if (taskTypeFilter === 'active') {
            matchesTaskType = taskTypeValue === 'active';
        } else if (taskTypeFilter === 'inactive') {
            matchesTaskType = taskTypeValue === 'inactive';
        }

        // Show/hide row
        if (matchesSearch && matchesTaskType) {
...
function restoreTableIfNeeded() {
    const container = document.querySelector('.task-type-table-container');
    const existingTable = container.querySelector('.task-type-table');
...
function toggleTaskType(id, isActive) {
    if (!confirm('Are you sure you want to ' + (isActive ? 'activate' : 'deactivate') + ' this task type?')) {
...
function checkEmptyState() {
    ...
    const container = document.querySelector('.task-type-table-container');
    const tableContainer = container.querySelector('.task-type-table');
    ...
    if (visibleRows.length === 0 && tableContainer) {
        ...
        if (container.dataset.originalTable) {
            ...
            const tbody = container.querySelector('.task-type-table tbody');
            if (tbody) {
                allRows = Array.from(tbody.querySelectorAll('tr'));
            }
        }
...
<div class="task-type-container">
    <div class="task-type-header">
        ...
        <?= Html::a('<span>➕</span> Create Task Type', ['create'], ['class' => 'btn-create']) ?>
    </div>

    <!-- Search and Filter Section -->
    <div class="filter-section" style="margin-bottom: 20px;">
        <div class="search-box" style="max-width: 500px;">
            <input type="text" id="searchInput" class="search-input" placeholder="Search by task type name or description..." style="width: 100%; padding: 12px 16px; border: 2px solid #D4A574; border-radius: 8px; font-size: 14px;">
        ...
        <div class="filter-controls" style="display: flex; gap: 12px; margin-top: 12px;">
            <select id="taskTypeFilter" class="filter-select" style="padding: 10px 16px; border: 2px solid #D4A574; border-radius: 8px; font-size: 14px; color: #2D1F13; background: white; cursor: pointer;">
                <option value="all">All Task Type</option>
    ...
    <div class="task-type-table-container">
                ...
                <tbody>
                    <?php foreach ($dataProvider->getModels() as $model): ?>
                        <tr>
                            ...
                            <td>
                                <div class="task-type-badge" style="background-color: <?= Html::encode($model->color) ?>;">
                            ...
                            <td data-task-type="<?= $model->is_active ? 'active' : 'inactive' ?>">
                                <label class="toggle-switch" data-id="<?= $model->id ?>" title="Click to toggle task type">
                                    <input type="checkbox" <?= $model->is_active ? 'checked' : '' ?> onchange="toggleTaskType(<?= $model->id ?>, this.checked)">
                            ...
                            <td>
                                <div class="action-buttons">
                                    ...
                                    <?= Html::a('<span>🗑</span> Delete', ['delete', 'id' => $model->id], [
                                        ...
                                        'data' => [
                                            'confirm' => 'Are you sure you want to delete this task type?',
                                            'method' => 'post',
                                        ],
                                    ]) ?>
                                </div>
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <div class="empty-state-text">No task type records found</div>
                <div class="empty-state-subtext">Click "Create Task Type" to add your first task type</div>
            </div>
        <?php endif; ?>
    </div>
</div>
