...
$this->title = 'Task Category Management';
?>
<style>
    ...
    .task-category-table thead th:first-child {
        border-radius: 10px 0 0 0;
    }

    .task-category-table thead th:last-child {
        border-radius: 0 10px 0 0;
    }

    .task-category-table tbody tr {
        transition: all 0.3s ease;
    }

    .task-category-table tbody tr:hover {
        background: #f8f5f1;
        transform: scale(1.01);
    }

    .task-category-table tbody td {
        padding: 16px;
    ...
    .task-category-table tbody tr:last-child td {
        border-bottom: none;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const tbody = document.querySelector('.task-category-table tbody');
    if (tbody) {
...
function applyFilters() {
    ...
    const container = document.querySelector('.task-category-table-container');
    
    // If all filters are empty/default, show all rows
    if (searchTerm === '' && statusFilter === 'all' && taskTypeFilter === 'all') {
    ...
    allRows.forEach(row => {
        ...
        const taskTypeId = row.getAttribute('data-task-type-id') || '';

        // Check search term (if empty, matches all)
        const matchesSearch = searchTerm === '' || categoryName.includes(searchTerm) || description.includes(searchTerm);
        ...
        const matchesTaskType = taskTypeFilter === 'all' || taskTypeId === taskTypeFilter;
...
function restoreTableIfNeeded() {
    const container = document.querySelector('.task-category-table-container');
    const existingTable = container.querySelector('.task-category-table');
...
function toggleStatus(id, isActive) {
    if (!confirm('Are you sure you want to ' + (isActive ? 'activate' : 'deactivate') + ' this task category?')) {
...
function checkEmptyState() {
    ...
    const container = document.querySelector('.task-category-table-container');
    const tableContainer = container.querySelector('.task-category-table');
    ...
    if (visibleRows.length === 0 && tableContainer) {
        ...
        if (container.dataset.originalTable) {
            ...
            const tbody = container.querySelector('.task-category-table tbody');
            if (tbody) {
                allRows = Array.from(tbody.querySelectorAll('tr'));
            }
        }
...
<div class="task-category-container">
    <div class="task-category-header">
        <h1>
            <span>📚</span>
            Task Category Library
        </h1>
        <?= Html::a('<span>➕</span> Create Task Category', ['create'], ['class' => 'btn-create']) ?>
    </div>

    <!-- Search and Filter Section -->
    <div class="filter-section" style="margin-bottom: 20px;">
        <div class="search-box" style="max-width: 500px;">
            <input type="text" id="searchInput" class="search-input" placeholder="Search by category name or description..." style="width: 100%; padding: 12px 16px; border: 2px solid #D4A574; border-radius: 8px; font-size: 14px;">
        ...
        <div class="filter-controls" style="display: flex; gap: 12px; margin-top: 12px;">
            <select id="taskTypeFilter" class="filter-select" style="padding: 10px 16px; border: 2px solid #D4A574; border-radius: 8px; font-size: 14px; color: #2D1F13; background: white; cursor: pointer;">
                ...
                <?php foreach (TaskType::getDropdownList() as $id => $name): ?>
    ...
    <div class="task-category-table-container">
            ...
            <table class="task-category-table">
                ...
                <tbody>
                    <?php foreach ($dataProvider->getModels() as $model): ?>
                        <tr data-task-type-id="<?= $model->task_type_id ?>">
                            <td><?= Html::encode($model->id) ?></td>
                            <td>
                                <span class="task-type-badge">
                                    <?= Html::encode($model->taskType ? $model->taskType->name : 'N/A') ?>
                                </span>
                            </td>
                            <td>
                                <div class="task-category-badge" style="background-color: <?= Html::encode($model->color) ?>;">
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <div class="empty-state-text">No task category records found</div>
                <div class="empty-state-subtext">Click "Create Task Category" to add your first category</div>
            </div>
        <?php endif; ?>
    </div>
</div>
