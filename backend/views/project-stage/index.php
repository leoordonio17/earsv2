...
$this->title = 'Project Stage Management';
?>
<style>
    ...
    .project-stage-table thead th:first-child {
        border-radius: 10px 0 0 0;
    }

    .project-stage-table thead th:last-child {
        border-radius: 0 10px 0 0;
    }

    .project-stage-table tbody tr {
        transition: all 0.3s ease;
    }

    .project-stage-table tbody tr:hover {
        background: #f8f5f1;
        transform: scale(1.01);
    }

    .project-stage-table tbody td {
        padding: 16px;
    ...
    .project-stage-table tbody tr:last-child td {
        border-bottom: none;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const tbody = document.querySelector('.project-stage-table tbody');
    if (tbody) {
...
function applyFilters() {
    ...
    const container = document.querySelector('.project-stage-table-container');
    ...
    allRows.forEach(row => {
        ...
        const matchesSearch = searchTerm === '' || stageName.includes(searchTerm) || description.includes(searchTerm);
...
function restoreTableIfNeeded() {
    const container = document.querySelector('.project-stage-table-container');
    const existingTable = container.querySelector('.project-stage-table');
...
function toggleProjectStage(id, isActive) {
    if (!confirm('Are you sure you want to ' + (isActive ? 'activate' : 'deactivate') + ' this project stage?')) {
...
function checkEmptyState() {
    ...
    const container = document.querySelector('.project-stage-table-container');
    const tableContainer = container.querySelector('.project-stage-table');
    ...
    if (visibleRows.length === 0 && tableContainer) {
        ...
        if (container.dataset.originalTable) {
            ...
            const tbody = container.querySelector('.project-stage-table tbody');
            if (tbody) {
                allRows = Array.from(tbody.querySelectorAll('tr'));
            }
        }
...
<div class="project-stage-container">
    <div class="project-stage-header">
        <h1>
            <span>📚</span>
            Project Stage Library
        </h1>
        <?= Html::a('<span>➕</span> Create Project Stage', ['create'], ['class' => 'btn-create']) ?>
    </div>

    <!-- Search and Filter Section -->
    <div class="filter-section" style="margin-bottom: 20px;">
        <div class="search-box" style="max-width: 500px;">
            <input type="text" id="searchInput" class="search-input" placeholder="Search by project stage name or description..." style="width: 100%; padding: 12px 16px; border: 2px solid #D4A574; border-radius: 8px; font-size: 14px;">
    ...
    <div class="project-stage-table-container">
                ...
                <tbody>
                    <?php foreach ($dataProvider->getModels() as $model): ?>
                        <tr>
                            ...
                            <td>
                                <div class="project-stage-badge" style="background-color: <?= Html::encode($model->color) ?>;">
                            ...
                            <td data-status="<?= $model->is_active ? 'active' : 'inactive' ?>">
                                <label class="toggle-switch" data-id="<?= $model->id ?>" title="Click to toggle status">
                                    <input type="checkbox" <?= $model->is_active ? 'checked' : '' ?> onchange="toggleProjectStage(<?= $model->id ?>, this.checked)">
                            ...
                            <td>
                                <div class="action-buttons">
                                    ...
                                    <?= Html::a('<span>🗑</span> Delete', ['delete', 'id' => $model->id], [
                                        ...
                                        'data' => [
                                            'confirm' => 'Are you sure you want to delete this project stage?',
                                            'method' => 'post',
                                        ],
                                    ]) ?>
                                </div>
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <div class="empty-state-text">No project stage records found</div>
                <div class="empty-state-subtext">Click "Create Project Stage" to add your first project stage</div>
            </div>
        <?php endif; ?>
    </div>
</div>
